...
function getDiaochan(player, info)
    local room = player:getRoom()
    local all_players = room:getAllPlayers()
    local liege = nil
    for _, p in sgs.qlist(all_players) do
        if p:getTag("Meihuo"):toInt() == 1 then liege = p end
    end
    local msg = sgs.LogMessage()
    msg.type = "%from ask %to for "
    if info then msg.type = msg.type..info end
    msg.from = player
    msg.to:append(liege)
    room:sendLog(msg)
    return liege
...
function setInitialTables()
    ...
    sgs.priority_skill =        "dimeng|haoshi|qingnang|nosjizhi|jizhi|guzheng|qixi|jieyin|guose|duanliang|jujian|fanjian|neofanjian|lijian|yuqi|" ..
    ...
    sgs.exclusive_skill =       "huilei|duanchang|wuhun|buqu|dushi|wuchang"
...
sgs.ai_compare_funcs = {
    ...
    huoxin = function(a, b)
        local am, bm = a:getMark("@Mei"), b:getMark("@Mei")
        if am == bm then
...
suitMeihuo = function(player,reason)
	if player:getTag("Meihuoing"):toInt() == 1 then
        local room = player:getRoom()
		local liege = getDiaochan(player, "suit")
		local slash = room:askForSuit(liege, reason)
		return slash
	end
	return nil
end
function SmartAI:askForSuit(reason)
	local pre = suitMeihuo(self.player,reason)
...
invokeMeihuo = function(player,skill_name, data)
	if player:getTag("Meihuoing"):toInt() == 1 then
        local room = player:getRoom()
		local liege = getDiaochan(player, "invoke "..skill_name)
		local slash = room:askForSkillInvoke(liege, skill_name, data)
		return slash
	end
	return nil
end
function SmartAI:askForSkillInvoke(skill_name, data)
	local pre = invokeMeihuo(self.player,skill_name, data)
...
choiceMeihuo = function(player,skill_name, choices, data)
	if player:getTag("Meihuoing"):toInt() == 1 then
        local room = player:getRoom()
		local liege = getDiaochan(player, "choice")
		local slash = room:askForChoice(liege, skill_name, choices, data)
		return slash
	end
	return nil
end
function SmartAI:askForChoice(skill_name, choices, data)
	local pre = choiceMeihuo(self.player,skill_name, choices, data)
	if pre then return pre end
...
discardMeihuo = function(player, reason, discard_num, min_num, optional, include_equip)
	if player:getTag("Meihuoing"):toInt() == 1 then
        -- player:drawCards(1)
        local room = player:getRoom()
		local liege = getDiaochan(player,"discard")
        local lis = {}
        ...
        for i=0, discard_num-1 do
            ...
            local slash = room:askForAG(liege, cardlist, false, "")
            room:clearAG()
            -- player:drawCards(1)
            if slash >= 0 then 
                table.insert(lis, slash)
                cardlist:removeOne(slash)
            elseif cardlist:isEmpty() then
                return lis
            else i = i-1 end
        end
        return lis
...
function SmartAI:askForDiscard(reason, discard_num, min_num, optional, include_equip)
	local pre = discardMeihuo(self.player,reason, discard_num, min_num, optional, include_equip)
	if pre then return pre end
    ...
    self:sortByKeepValue(cards,player:getTag("Meihuo"):toInt() == 1)
    ...
    for _, card in ipairs(cards) do
        if exchange or not self.player:isJilei(card) then
            ...
            elseif (player:getTag("Meihuo"):toInt() == 1) == (self:getKeepValue(card) >= 4.1) then
                table.insert(temp, card:getEffectiveId())
            else
...
chosenMeihuo = function(player,who, flags, reason, method)
	if player:getTag("Meihuoing"):toInt() == 1 then
        local room = player:getRoom()
		local liege = getDiaochan(player,"card chosen")
		local slash = room:askForCardChosen(liege, who, flags, reason, method)
		return slash
	end
	return nil
end
function SmartAI:askForCardChosen(who, flags, reason, method)
	local pre = chosenMeihuo(self.player,who, flags, reason, method)
	if pre then return pre end
...
cardMeihuo = function(player,pattern, prompt, data)
	if player:getTag("Meihuoing"):toInt() == 1 then
        if pattern == "jink" or pattern == "slash" or pattern == "peach" or pattern == "nullification" then
            return nil
        end
        local room = player:getRoom()
		local liege = getDiaochan(player, pattern)
        local cardlist = sgs.IntList()
        local cards = player:getHandcards()
        ...
        for _,c in sgs.qlist(equips) do cardlist:append(c:getEffectiveId()) end
        room:fillAG(cardlist)
        local slash = room:askForAG(liege, cardlist, true, "")
        room:clearAG()
        -- liege:drawCards(3)
        -- local msg = sgs.LogMessage()
        -- msg.type = type(sgs.Sanguosha:getCard(slash):get)
        -- room:sendLog(msg)
        -- local msg = sgs.LogMessage()
        -- msg.type = pattern
        -- room:sendLog(msg)
        ...
        local msg = sgs.LogMessage()
        msg.type = "$"..tostring(slash)
        room:sendLog(msg)
        -- if sgs.Sanguosha:getCard(slash):match(pattern) then
        local liege = getDiaochan(player, "zhehendui")
        if slash >= 0 then return slash
...
function SmartAI:askForCard(pattern, prompt, data)
	local pre = cardMeihuo(self.player,pattern, prompt, data)
    ...
    -- local msg = sgs.LogMessage()
    -- msg.type = card.." 456456"
    -- room:sendLog(msg)
    return card
end

useMeihuo = function(self, pattern, prompt, method)
	if self.player:getTag("Meihuoing"):toInt() == 1 then
        -- player:drawCards(1)
        local room = self.player:getRoom()
		local liege = getDiaochan(self.player, pattern)
        -- local lis = {}
        -- local cardlist = sgs.IntList()
        -- local cards = player:getHandcards()
        -- local equips = player:getEquips()
        ...
        --     else i = i-1 end
        -- end
        local chosen = room:askForPlayerChosen(liege, room:getAllPlayers(), nil)
        local func = sgs.ai_skill_use[pattern]
        local should = func(self, prompt, method)
        local analyze = should:split("->")
        return analyze[1].."->"..chosen:objectName()
        -- return lis
	end
end
function SmartAI:askForUseCard(pattern, prompt, method)
	local pre = useMeihuo(self, pattern, prompt, method)
	if pre then return pre end
...
AGMeihuo = function(player, card_ids, refusable)
    -- player:drawCards(1)
	if player:getTag("Meihuoing"):toInt() == 1 then
        local room = player:getRoom()
		local liege = getDiaochan(player, "AG")
        local lis = sgs.IntList()
        for i=1, #card_ids do lis:append(card_ids[i]) end
		local slash = room:askForAG(liege, lis, refusable, "")
		return slash
	end
	return nil
end
function SmartAI:askForAG(card_ids, refusable, reason)
    local pre = AGMeihuo(self.player, card_ids, refusable)
	if pre then return pre end
...
showMeihuo = function(player)
	if player:getTag("Meihuoing"):toInt() == 1 then
		...
		local liege = getDiaochan(player, "show")
		room:fillAG(cardlist)
		local slash = room:askForAG(liege, cardlist, false, "")
		room:clearAG()
		return sgs.Sanguosha:getCard(slash)
	end
	return nil
end
...
function SmartAI:askForPindian(requestor, reason)
	local pre = showMeihuo(self.player)
	if pre then return pre end
...
playerMeihuo = function(player,targets, reason)
	if player:getTag("Meihuoing"):toInt() == 1 then
        local room = player:getRoom()
		local liege = getDiaochan(player, "choose player")
        local chosen = room:askForPlayerChosen(liege, targets, reason)
		return chosen
	end
	return nil
end
function SmartAI:askForPlayerChosen(targets, reason)
	local pre = playerMeihuo(self.player,targets, reason)
	if pre then return pre end
...
retrialMeihuo = function(player,judge)
	if player:getTag("Meihuoing"):toInt() == 1 then
        local room = player:getRoom()
		local liege = getDiaochan(player, "retrial")
        -- liege:drawCards(3)
		local chosen = room:askForChoice(liege, judge.reason, "retrial+not retrial")
        return chosen == "retrial"
	end
	return nil
end
...
function SmartAI:needRetrial(judge)	
    ...
    -- local pre = retrialMeihuo(self.player,judge)
	-- if pre ~= nil then return pre end
    local reason = judge.reason
    local lord = getLord(self.player)
    local who = judge.who
