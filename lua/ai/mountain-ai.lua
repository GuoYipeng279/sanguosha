...
function sgs.ai_skill_choice.huashen(self, choices, data, xiaode_choice)
	...
	if (xiaode_choice and xiaode_choice > 0) or self.player:getPhase() == sgs.Player_RoundStart then
		...
		"shouye|shaoying|xingshang|suishi|yuwen|gongmou|weiwudi_guixin|wuling|yuyi|xieyou|nichang|shenfen|cancel"):split("|")) do
			if str:matchOne(askill) then return askill end
		end
	else
		...
		if not (self.player:isWounded()) and str:matchOne("nichang") and self.player:getPile("hufu"):length() >= 5 then return "nichang" end
		...
		if str:matchOne("yuyi") then
			local adjFriend = 0
			for _,p in ipairs(self.friends_noself) do
				if self.player:isAdjacentTo(p) then adjFriend = adjFriend + 1 end
			end
			if adjFriend >= 2 then return "yuyi" end
		end
		...
		if str:matchOne("cancel") then return "cancel" end
	end
